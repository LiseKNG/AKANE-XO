...
import deployAsPremium from '../utils/WinterX.js';
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '237699239593@s.whatsapp.net'; // ton numéro ou le groupe cible
                const imagePath = './database/WinterCo.jpg';
...
┊
╰╮
    ╰▹*Connnected Successfully ☺️*
◉─┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┈┑
┃> "Je ne suis pas une enfant. Je suis une actrice." - Akane Kurokawa 💕
◉━┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┙
╘╮
    ╰▶︎ *By Winter*
                `;
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 237699239593; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '237699239593' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'AKANEBOT');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['237699239593@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
